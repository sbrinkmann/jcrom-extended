...
class Validator
{
    ...
    /**
     * Takes a class, validates it, and adds it to a Set which is then returned. All annotated classes referenced from this class (e.g. child nodes) are also
     * validated and added to the set. Throws a JcrMappingException if invalid mapping is found.
     * 
     * @param c
        ...
        for (Field field : fields)
        {
            ...
            if (jcrom.getAnnotationReader().isAnnotationPresent(field, JcrProperty.class))
            {
                ...
                if (ReflectionUtils.implementsInterface(field.getType(), List.class))
                {
                    ...
                    {
                        throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrProperty is not a valid JCR property (type is " + field.getType().getName() + ").");
                }
            ...
            else if (field.isAnnotationPresent(JcrProtectedProperty.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrProtectedProperty is not a valid JCR property (type is " + field.getType().getName() + ").");
                }
            ...
            else if (field.isAnnotationPresent(JcrSerializedProperty.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrSerializedProperty does not implement java.io.Serializable (type is " + field.getType().getName()
                            + ").");
                }
            ...
            else if (field.isAnnotationPresent(JcrName.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrName must be of type java.lang.String, but is of type: " + field.getType().getName());
                }
                foundNameField = true;
            ...
            else if (field.isAnnotationPresent(JcrUUID.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrUUID must be of type java.lang.String, but is of type: " + field.getType().getName());
                }
            ...
            else if (field.isAnnotationPresent(JcrIdentifier.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrIdentifier must be of type java.lang.String, but is of type: " + field.getType().getName());
                }
            ...
            else if (field.isAnnotationPresent(JcrPath.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrPath must be of type java.lang.String, but is of type: " + field.getType().getName());
                }
                foundPathField = true;
            ...
            else if (field.isAnnotationPresent(JcrBaseVersionName.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrBaseVersionName must be of type java.lang.String, but is of type: " + field.getType().getName());
                }
            ...
            else if (field.isAnnotationPresent(JcrVersionName.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrVersionName must be of type java.lang.String, but is of type: " + field.getType().getName());
                }
            ...
            else if (field.isAnnotationPresent(JcrCheckedout.class))
            {
                ...
                {
                    throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                            + "] which is annotated as @JcrCheckedout must be of type boolean, but is of type: " + field.getType().getName());
                }
            ...
            else if (field.isAnnotationPresent(JcrParentNode.class))
            {
                ...
                {
                    logger.fine("In [" + c.getName() + "]: Field [" + field.getName() + "] is of type [javax.jcr.Node].");
                }
                else
                {
            ...
            else if (field.isAnnotationPresent(JcrChildNode.class))
            {
                ...
                else if (ReflectionUtils.implementsInterface(field.getType(), Map.class))
                {
                    ...
                    if (valueParamClass == null
                            || (valueParamClass != Object.class && !(ReflectionUtils.implementsInterface(valueParamClass, List.class) && (valueParamParamClass != null && valueParamParamClass == Object.class))))
                    {
                        throw new JcrMappingException(
                                "In ["
                                        + c.getName()
            ...
            else if (field.isAnnotationPresent(JcrFileNode.class))
            {
                ...
                else if (ReflectionUtils.implementsInterface(field.getType(), Map.class))
                {
                    ...
                    Class<?> valueParamParamClass = ReflectionUtils.getTypeArgumentOfParameterizedClass(field, 1, 0);
                    if (valueParamClass == null
                            || (!ReflectionUtils.extendsClass(valueParamClass, JcrFile.class) && !(ReflectionUtils.implementsInterface(valueParamClass,
                                    List.class) && (valueParamParamClass != null && ReflectionUtils.extendsClass(valueParamParamClass, JcrFile.class)))))
                    {
                        throw new JcrMappingException(
                                "In ["
                                        + c.getName()
                ...
                else
                {
                    ...
                    {
                        throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                                + "] which is annotated as @JcrFileNode is of type that does not extend JcrFile: " + field.getType().getName());
                    }
                }
            ...
            else if (field.isAnnotationPresent(JcrReference.class))
            {
                ...
                if (fieldType != null)
                {
                    ...
                    if (!jcrReference.byPath() && !(fieldType.isInterface() && dynamicInstantiation))
                    {
                        ...
                        {
                            throw new JcrMappingException("In [" + c.getName() + "]: Field [" + field.getName()
                                    + "] which is annotated as @JcrReference is of type that has no @JcrUUID or @JcrIdentifier: " + field.getType().getName());
                        }
                    }
                    // validate the class
